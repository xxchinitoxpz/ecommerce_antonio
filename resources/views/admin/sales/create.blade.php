<x-layouts.app :title="'POS - Nueva venta'">
    <div class="grid grid-cols-12 gap-4">
        <!-- 🧾 Detalle de venta -->
        <div
            class="col-span-12 md:col-span-4 bg-white dark:bg-zinc-800 p-4 rounded shadow border dark:border-zinc-700 flex flex-col">
            <h2 class="text-lg font-semibold mb-4 text-gray-800 dark:text-white border-b">Detalle de Venta</h2>
            ...
            <form action="{{ route('admin.sales.store') }}" method="POST">
                ...
                <button type="submit" class="btn btn-blue w-full mt-4">Registrar venta</button>
            </form>
        </div>

        <!-- 🛍️ Productos -->
        <div
            class="col-span-12 md:col-span-8 bg-white dark:bg-zinc-800 p-4 rounded shadow border dark:border-zinc-700 flex flex-col">
            <div class="mb-4">
                    ...
                    @foreach ($products->where('stock', '>', 0) as $product)
                        <div class="bg-white dark:bg-zinc-700 p-3 rounded shadow hover:scale-105 transition cursor-pointer product-card"
                            ...
                            <h3 class="text-base font-semibold text-gray-800 dark:text-white truncate">
                                {{ $product->name }}</h3>
                            <p class="text-base">
                                <span class="line-through text-red-400">S/.
    ...
    @push('js')
        <script>
            ...
            function addToCart(id, name, price) {
                ...
                const stock = getProductStock(id); // 👇 te explico abajo
